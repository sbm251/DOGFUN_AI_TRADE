...
class AccountValueChart(ttk.Frame):
    def __init__(self, parent: tk.Widget, history_path: str, trade_history_path: str, trader_status_path: str = None, max_points: int = 250):
        super().__init__(parent)
    ...
    def refresh(self) -> None:
        ...
        points_total: List[Tuple[float, float]] = []
        points_holdings: List[Tuple[float, float]] = []
            ...
            if os.path.isfile(path):
                ...
                for ln in lines:
                    try:
                        obj = json.loads(ln)
                        ts = obj.get("ts", None)
                        total_v = obj.get("total_account_value", None)
                        holdings_v = obj.get("holdings_sell_value", None)
                        ...
                        # Add total account value point
                        if total_v is not None:
                            vf = float(total_v)
                            if math.isfinite(tsf) and math.isfinite(vf) and vf >= 0.0:
                                points_total.append((tsf, vf))
                        
                        # Add holdings value point
                        if holdings_v is not None:
                            vf = float(holdings_v)
                            if math.isfinite(tsf) and math.isfinite(vf) and vf >= 0.0:
                                points_holdings.append((tsf, vf))
                    except Exception:
                        continue
        except Exception:
            points_total = []
        ...
        # Use total account value points as primary (for downsampling and x-axis)
        points = points_total
        ...
        # Clean up holdings points similarly
        if points_holdings:
            points_holdings.sort(key=lambda x: x[0])
            dedup_holdings: List[Tuple[float, float]] = []
            for tsf, vf in points_holdings:
                if dedup_holdings and tsf == dedup_holdings[-1][0]:
                    dedup_holdings[-1] = (tsf, vf)
                else:
                    dedup_holdings.append((tsf, vf))
            points_holdings = dedup_holdings
        ...
        if n > max_keep:
            ...
            # Downsample holdings to match the same timestamps as total account value
            if points_holdings:
                # Create a dict for quick lookup
                holdings_dict = {h[0]: h[1] for h in points_holdings}
                
                # For each downsampled timestamp, find the nearest holdings value
                new_holdings: List[Tuple[float, float]] = []
                for ts, _ in points:
                    if ts in holdings_dict:
                        new_holdings.append((ts, holdings_dict[ts]))
                    else:
                        # Find nearest holdings timestamp
                        nearest = min(points_holdings, key=lambda h: abs(h[0] - ts)) if points_holdings else None
                        if nearest and abs(nearest[0] - ts) < 10:  # Only use if within 10 seconds
                            new_holdings.append((ts, nearest[1]))
                        else:
                            # Use 0 if no nearby value
                            new_holdings.append((ts, 0.0))
                
                points_holdings = new_holdings
        ...
        # If no history, try to read current value from trader_status.json
        if not points and not points_holdings and self.trader_status_path:
            try:
                if os.path.isfile(self.trader_status_path):
                    with open(self.trader_status_path, 'r', encoding='utf-8') as f:
                        status = json.load(f)
                        total_val = status.get('account', {}).get('total_account_value')
                        holdings_val = status.get('account', {}).get('holdings_sell_value')
                        
                        if total_val is not None:
                            current_time = time.time()
                            points = [(current_time, float(total_val))]
                            if holdings_val is not None:
                                points_holdings = [(current_time, float(holdings_val))]
            except Exception:
                pass
        
        if not points and not points_holdings:
            self.ax.set_title("Account Value - no data", color=DARK_FG)
            self.last_update_label.config(text="Last: N/A")
            self.canvas.draw_idle()
            return
        
        # Update title to show both values
        if points:
            latest_total = points[-1][1] if points else 0.0
            latest_holdings = points_holdings[-1][1] if points_holdings and len(points_holdings) == len(points) else 0.0
            self.ax.set_title(f"Account Value (Total: {latest_total:.2f} EUR, Holdings: {latest_holdings:.2f} EUR)", color=DARK_FG)
            ...
            if points:
                ...
                try:
                    ...
                    self.last_update_label.config(text=f"Last: {last_str}")
                except Exception:
                    self.last_update_label.config(text="Last: N/A")
        ...
        if points:
            ...
            ys_total = [round(p[1], 2) for p in points]
            
            # Plot total account value line (cyan/blue)
            self.ax.plot(xs, ys_total, linewidth=1.5, color="#00E5FF", label="Total Account Value", alpha=0.9)
            
            # Plot holdings value line (orange) - use already-aligned downsampled holdings
            if points_holdings and len(points_holdings) == len(points):
                ys_holdings = [round(h[1], 2) for h in points_holdings]
                self.ax.plot(xs, ys_holdings, linewidth=1.5, color="#FFA500", label="Holdings Value (Sellable)", alpha=0.9)
            
            # Add legend
            self.ax.legend(loc='upper left', facecolor=DARK_PANEL, edgecolor=DARK_BORDER, labelcolor=DARK_FG, fontsize=9)
        elif points_holdings:
            # Fallback if only holdings data exists
            xs = list(range(len(points_holdings)))
            ys_holdings = [round(p[1], 2) for p in points_holdings]
            self.ax.plot(xs, ys_holdings, linewidth=1.5, color="#FFA500", label="Holdings Value")
            self.ax.legend(loc='upper left', facecolor=DARK_PANEL, edgecolor=DARK_BORDER, labelcolor=DARK_FG)

        # --- Trade dots (BUY / DCA / SELL) for ALL coins ---
        try:
            trades = _read_trade_history_jsonl(self.trade_history_path) if self.trade_history_path else []
            if trades and points:  # Only show trade dots if we have points to plot
                ...
                for tr in trades:
                    ...
                    x = idx
                    # Use total account value for trade dot placement
                    if ys_total and idx < len(ys_total):
                        y = ys_total[idx]
                    elif ys_holdings and idx < len(ys_holdings):
                        y = ys_holdings[idx]
                    else:
                        continue  # Skip if no valid y value
        ...
        if points:
            ...
            self.ax.set_xlim(-0.5, (len(points_holdings) - 0.5) + 0.6)
        
        # Title and last update are already set earlier, no need to override
...
class PowerTraderHub(tk.Tk):
    def __init__(self):
        ...
        # Process management for Telegram bot
        self.proc_telegram_bot = ProcInfo(
            name="Telegram Bot",
            path=os.path.abspath(os.path.join(self.project_dir, "telegram_bot.py"))
        )
        ...
        # Show login dialog before starting anything
        self.update()
    ...
    def _ensure_alt_coin_folders_and_trainer_on_startup(self) -> None:
        ...
        - For every alt coin in the coin list that does NOT have its folder yet:
            - create the folder
            - copy pt_trainer.py from the MAIN (BTC) folder into the new folder
        - Also ensure the trainer exists in the main_neural_dir
        """
        try:
            ...
            # Source trainer: Try project directory first, then configured path
            src_trainer_path = None
            
            # Try project directory
            project_trainer = os.path.join(self.project_dir, trainer_name)
            if os.path.isfile(project_trainer):
                src_trainer_path = project_trainer
            
            # Try configured path
            if not src_trainer_path:
                cfg_path = str(self.settings.get("script_neural_trainer", trainer_name))
                if os.path.isabs(cfg_path) and os.path.isfile(cfg_path):
                    src_trainer_path = cfg_path
                elif os.path.isfile(os.path.join(self.project_dir, cfg_path)):
                    src_trainer_path = os.path.join(self.project_dir, cfg_path)
            
            # If we found a trainer, ensure it's in the main_dir
            if src_trainer_path and os.path.isfile(src_trainer_path):
                main_trainer_path = os.path.join(main_dir, trainer_name)
                if not os.path.isfile(main_trainer_path):
                    try:
                        # Ensure main_dir exists
                        os.makedirs(main_dir, exist_ok=True)
                        shutil.copy2(src_trainer_path, main_trainer_path)
                        print(f"[STARTUP] Copied trainer to main_dir: {main_trainer_path}")
                    except Exception as e:
                        print(f"[STARTUP] Could not copy trainer to main_dir: {e}")

            # Use main_dir trainer as source for alt coins
            src_main_trainer = os.path.join(main_dir, trainer_name)
            if not os.path.isfile(src_main_trainer) and src_trainer_path:
                src_main_trainer = src_trainer_path

            # Also ensure telegram_notifications.py is in main_dir
            telegram_notif_name = "telegram_notifications.py"
            src_telegram_notif = None
            if src_trainer_path:
                # Try project directory
                project_telegram = os.path.join(self.project_dir, telegram_notif_name)
                if os.path.isfile(project_telegram):
                    src_telegram_notif = project_telegram
            
            if src_telegram_notif:
                main_telegram_path = os.path.join(main_dir, telegram_notif_name)
                if not os.path.isfile(main_telegram_path):
                    try:
                        shutil.copy2(src_telegram_notif, main_telegram_path)
                        print(f"[STARTUP] Copied telegram_notifications.py to main_dir: {main_telegram_path}")
                    except Exception as e:
                        print(f"[STARTUP] Could not copy telegram_notifications.py to main_dir: {e}")
            ...
            for coin in coins:
                ...
                if created:
                    ...
                    if (not os.path.isfile(dst_trainer_path)) and os.path.isfile(src_main_trainer):
                        shutil.copy2(src_main_trainer, dst_trainer_path)
                    
                    # Also copy telegram_notifications.py
                    if src_telegram_notif:
                        dst_telegram_notif = os.path.join(coin_dir, telegram_notif_name)
                        if not os.path.isfile(dst_telegram_notif):
                            try:
                                shutil.copy2(src_telegram_notif, dst_telegram_notif)
                            except Exception:
                                pass
        except Exception as e:
            print(f"[STARTUP] Error in _ensure_alt_coin_folders_and_trainer_on_startup: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _build_menu(self) -> None:
        ...
        m_scripts.add_separator()
        m_scripts.add_command(label="Start Telegram Bot", command=self.start_telegram_bot)
        m_scripts.add_command(label="Stop Telegram Bot", command=self.stop_telegram_bot)
        ...
        m_help.add_command(label="About...", command=self._show_about_dialog)
    ...
    def _show_about_dialog(self) -> None:
        """Show About dialog with logo image"""
        about_window = tk.Toplevel(self)
        about_window.title("About PowerTrader AI")
        ...
        # Logo
        logo_path = os.path.join(self.project_dir, "image.png")
        ...
        # About text
        about_text = """PowerTrader AI Hub

A comprehensive trading bot management system
for automated cryptocurrency trading.

Features:
• Automated trading with AI predictions
• Neural network training for multiple coins
...
Supported by DogFun"""
    ...
    def _build_layout(self) -> None:
        ...
        # Test Buy button
        self.btn_test_buy = ttk.Button(
            start_all_row,
            text="Test Buy",
            width=BTN_W,
            command=self.test_buy_coin,
        )
        self.btn_test_buy.pack(side="left", padx=(6, 0))
        
        # Sell All button
        self.btn_sell_all = ttk.Button(
            start_all_row,
            text="Sell All",
            width=BTN_W,
            command=self.sell_all_holdings,
        )
        self.btn_sell_all.pack(side="left", padx=(6, 0))
        
        # Buy/Sell coin selection section
        buy_sell_row = ttk.Frame(controls_left)
        buy_sell_row.pack(fill="x", padx=6, pady=(0, 6))
        
        # Buy coin selector
        ttk.Label(buy_sell_row, text="Buy Coin:").pack(side="left", padx=(0, 6))
        self.buy_coin_var = tk.StringVar(value="DOGE")
        self.buy_coin_combo = ttk.Combobox(buy_sell_row, textvariable=self.buy_coin_var, width=8, state="readonly")
        self.buy_coin_combo['values'] = tuple(self.coins) if self.coins else ("DOGE", "BTC", "ETH", "XRP", "BNB", "ADA")
        self.buy_coin_combo.pack(side="left", padx=(0, 6))
        self.buy_coin_combo.set(self.coins[0] if self.coins else "DOGE")
        
        # Sell coin selector
        ttk.Label(buy_sell_row, text="Sell Coin:").pack(side="left", padx=(12, 6))
        self.sell_coin_var = tk.StringVar(value="DOGE")
        self.sell_coin_combo = ttk.Combobox(buy_sell_row, textvariable=self.sell_coin_var, width=8, state="readonly")
        self.sell_coin_combo['values'] = tuple(self.coins) if self.coins else ("DOGE", "BTC", "ETH", "XRP", "BNB", "ADA")
        self.sell_coin_combo.pack(side="left", padx=(0, 6))
        self.sell_coin_combo.set(self.coins[0] if self.coins else "DOGE")
        
        self.btn_sell_coin = ttk.Button(
            buy_sell_row,
            text="Sell",
            width=BTN_W,
            command=self.sell_selected_coin,
        )
        self.btn_sell_coin.pack(side="left")
        ...
        # Trade amount setting (editable)
        trade_amount_row = ttk.Frame(acct_box)
        trade_amount_row.pack(fill="x", padx=6, pady=(2, 0))
        self.lbl_acct_trade_amount = ttk.Label(trade_amount_row, text="Trade Amount: N/A")
        self.lbl_acct_trade_amount.pack(side="left")
        self.btn_set_trade_amount = ttk.Button(
            trade_amount_row,
            text="Set",
            width=6,
        ...
        # Start account balance update loop after window is fully displayed
        # Use after_idle to ensure GUI is ready, then delay slightly for API calls
        self.after_idle(lambda: self.after(1000, self._update_account_balance))
    ...
    def start_all_scripts(self) -> None:
        ...
        # Filter out ignored coins when checking if all are trained
        ignored_coins = set([str(c).strip().upper() for c in (self.settings.get("ignored_coins", []) or []) if str(c).strip()])
        active_coins = [c for c in self.coins if c not in ignored_coins]
        all_trained = all(self._coin_is_trained(c) for c in active_coins) if active_coins else False
        if not all_trained:
    ...
    def _coin_is_trained(self, coin: str) -> bool:
        ...
        # Check trainer_status.json first
        try:
            st = _safe_read_json(os.path.join(folder, "trainer_status.json"))
            if isinstance(st, dict):
                state = str(st.get("state", "")).upper()
                # If currently training, it's not trained yet
                if state == "TRAINING":
                    return False
                # If finished/completed, it IS trained
                if state == "FINISHED" or state == "COMPLETED":
                    return True
        except Exception:
            pass

        # Check trainer_status.json for FINISHED state (most reliable)
        ...
        # Fallback: check trainer_last_training_time.txt
    ...
    def _running_trainers(self) -> List[str]:
        ...
        for c in self.coins:
            try:
                ...
                if isinstance(st, dict):
                    state = str(st.get("state", "")).upper()
                    # Only add to running list if state is TRAINING (not FINISHED or COMPLETED)
                    if state == "TRAINING":
                        # Double-check: if trainer_last_training_time.txt exists and is newer than status file,
                        # training might have completed but status file wasn't updated
                        stamp_path = os.path.join(folder, "trainer_last_training_time.txt")
                        
                        try:
                            if os.path.isfile(stamp_path) and os.path.isfile(status_path):
                                # If timestamp file is newer, training likely completed
                                if os.path.getmtime(stamp_path) > os.path.getmtime(status_path):
                                    # Check if timestamp is recent (within last hour) - if old, training is done
                                    stamp_age = time.time() - os.path.getmtime(stamp_path)
                                    if stamp_age > 3600:  # More than 1 hour old
                                        continue  # Training completed, don't add to running
                        except Exception:
                            pass

                        running.append(coin)
                    # Explicitly skip coins with FINISHED or COMPLETED state
                    elif state in ("FINISHED", "COMPLETED"):
                        continue  # Don't add to running list
            except Exception:
                pass
    ...
    def start_trainer_for_selected_coin(self) -> None:
        ...
        # Ensure trainer exists in coin folder (for both BTC and alt coins)
        # BTC runs from main neural folder, alts run from their own coin subfolder
        try:
            if not os.path.isdir(coin_cwd):
                os.makedirs(coin_cwd, exist_ok=True)
            
            # Always ensure trainer is in the coin folder, copying from project directory if needed
            dst_trainer_path = os.path.join(coin_cwd, trainer_name)
            src_trainer_from_project = os.path.join(self.project_dir, trainer_name)
            
            # Copy trainer if it doesn't exist in coin folder, or if project has a newer version
            if not os.path.isfile(dst_trainer_path) or (os.path.isfile(src_trainer_from_project) and 
                os.path.getmtime(src_trainer_from_project) > os.path.getmtime(dst_trainer_path)):
                if os.path.isfile(src_trainer_from_project):
                    shutil.copy2(src_trainer_from_project, dst_trainer_path)
                    print(f"[TRAINER] Copied trainer to {coin} folder: {dst_trainer_path}")
            
            # Also copy telegram_notifications.py and credential files if they exist
            telegram_notif_name = "telegram_notifications.py"
            src_telegram_notif = os.path.join(self.project_dir, telegram_notif_name)
            if os.path.isfile(src_telegram_notif):
                dst_telegram_notif = os.path.join(coin_cwd, telegram_notif_name)
                if not os.path.isfile(dst_telegram_notif):
                    shutil.copy2(src_telegram_notif, dst_telegram_notif)
            
            # Copy Telegram credential files
            for cred_file in ["telegram_token.txt", "telegram_chat_id.txt"]:
                src_cred = os.path.join(self.project_dir, cred_file)
                if os.path.isfile(src_cred):
                    dst_cred = os.path.join(coin_cwd, cred_file)
                    if not os.path.isfile(dst_cred):
                        shutil.copy2(src_cred, dst_cred)
        except Exception as e:
            print(f"[TRAINER] Error setting up files for {coin}: {e}")
            import traceback
            traceback.print_exc()


        # Try to find trainer in coin folder first, then fall back to project directory
        trainer_path = os.path.join(coin_cwd, trainer_name)
        
        # If not found in coin folder, try project directory
        if not os.path.isfile(trainer_path):
            trainer_path = os.path.join(self.project_dir, trainer_name)
        
        # If still not found, try the configured script path directly
        if not os.path.isfile(trainer_path):
            configured_path = self.settings.get("script_neural_trainer", "pt_trainer.py")
            if os.path.isabs(configured_path) and os.path.isfile(configured_path):
                trainer_path = configured_path
            else:
                # Try relative to project directory
                trainer_path = os.path.join(self.project_dir, configured_path)

        if not os.path.isfile(trainer_path):
            messagebox.showerror(
                "Missing trainer",
                f"Cannot find trainer for {coin}.\n\nTried:\n"
                f"1. {os.path.join(coin_cwd, trainer_name)}\n"
                f"2. {os.path.join(self.project_dir, trainer_name)}\n"
                f"3. {self.settings.get('script_neural_trainer', 'pt_trainer.py')}\n\n"
                f"Please ensure pt_trainer.py exists in one of these locations."
            )
            return
        ...
        # Add project directory to PYTHONPATH so trainer can find telegram_notifications
        pythonpath = env.get("PYTHONPATH", "")
        if pythonpath:
            pythonpath = f"{self.project_dir}{os.pathsep}{pythonpath}"
        else:
            pythonpath = self.project_dir
    ...
    def _refresh_trader_status(self) -> None:
        ...
        if not data:
            ...
            try:
                ...
                # DCA affordability (only available when trader is running and writes trader_status.json)
                self.lbl_acct_dca_spread.config(text="DCA Levels (spread): N/A (trader not running)")
                self.lbl_acct_dca_single.config(text="DCA Levels (single): N/A (trader not running)")
            except Exception:
                pass
        ...
        try:
            ...
            # Update trade amount display (read from settings)
            try:
    ...
    def _refresh_pnl(self) -> None:
        ...
        if not data:
            self.lbl_pnl.config(text="Total realized: N/A (no trades yet)")
            return
    ...
    def open_settings_dialog(self) -> None:
        ...
        ignored_coins_var = tk.StringVar(value=",".join(self.settings.get("ignored_coins", [])))
        ...
        add_row(r, "Ignored coins (comma):", ignored_coins_var); r += 1
        ttk.Label(frm, text="(Coins to skip - insufficient data)", font=("TkDefaultFont", 8), foreground=DARK_MUTED).grid(row=r, column=1, sticky="w", padx=(0, 10)); r += 1
        ...
        # --- Binance API setup (writes binance_key.txt + binance_secret.txt used by pt_trader.py) ---
        ...
        def _refresh_api_status() -> None:
            ...
            missing = []
            if not k:
                missing.append("binance_key.txt (API Key)")
            if not s:
                missing.append("binance_secret.txt (API Secret)")
        ...
        def _open_api_folder() -> None:
            """Open the folder where binance_key.txt / binance_secret.txt live."""
            try:
        ...
        def _clear_api_files() -> None:
            """Delete binance_key.txt / binance_secret.txt (with a big confirmation)."""
            key_path, secret_path = _api_paths()
            if not messagebox.askyesno(
                ...
                f"  {key_path}\n"
                f"  {secret_path}\n\n"
                "After deleting, the trader can NOT authenticate until you configure them again.\n\n"
            ...
            _refresh_api_status()
            messagebox.showinfo("Deleted", "Deleted binance_key.txt and binance_secret.txt.")

        def _open_binance_api_setup() -> None:
            """
            Simple dialog to configure Binance API credentials.

            What we store:
              - binance_key.txt    = your Binance API Key
              - binance_secret.txt = your Binance API Secret (treat like a password — never share it)
            """
            import webbrowser
            ...
            intro = (
                ...
                "  - binance_secret.txt (API Secret) ← keep this secret!\n"
            )
            intro_lbl = ttk.Label(container, text=intro, justify="left")
            intro_lbl.grid(row=0, column=0, sticky="ew", pady=(0, 15))

            # API Key
            key_frame = ttk.LabelFrame(container, text="Binance API Key")
            key_frame.grid(row=1, column=0, sticky="ew", pady=(0, 10))
            key_frame.columnconfigure(0, weight=1)

            api_key_var = tk.StringVar(value=existing_api_key or "")
            ttk.Label(key_frame, text="API Key:").grid(row=0, column=0, sticky="w", padx=10, pady=(8, 4))
            api_key_entry = ttk.Entry(key_frame, textvariable=api_key_var, width=50)
            api_key_entry.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 10))

            # API Secret
            secret_frame = ttk.LabelFrame(container, text="Binance API Secret")
            secret_frame.grid(row=2, column=0, sticky="ew", pady=(0, 10))
            secret_frame.columnconfigure(0, weight=1)

            api_secret_var = tk.StringVar(value=existing_api_secret or "")
            ttk.Label(secret_frame, text="API Secret:").grid(row=0, column=0, sticky="w", padx=10, pady=(8, 4))
            api_secret_entry = ttk.Entry(secret_frame, textvariable=api_secret_var, width=50, show="*")
            api_secret_entry.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 10))
            ...
            def do_save():
                ...
                if not api_key:
                    messagebox.showerror("Missing API Key", "Please enter your Binance API Key.")
                    return
                if not api_secret:
                    messagebox.showerror("Missing API Secret", "Please enter your Binance API Secret.")
                    return
                ...
                messagebox.showinfo(
                    "Saved",
                    "✅ Saved!\n\n"
                    "The trader will automatically read these files:\n"
                    f"  API Key → {key_path}\n"
                    f"  API Secret → {secret_path}\n\n"
                    "Next: Start the trader (pt_trader.py)"
                )
                wiz.destroy()

            ttk.Button(btn_frame, text="Save", command=do_save).pack(side="left", padx=5)
            ttk.Button(btn_frame, text="Open Binance API Management", command=lambda: webbrowser.open("https://www.binance.com/en/my/settings/api-management")).pack(side="left", padx=5)
            ttk.Button(btn_frame, text="Close", command=wiz.destroy).pack(side="left", padx=5)
        ...
        ttk.Button(api_row, text="Setup", command=_open_binance_api_setup).grid(row=0, column=1, sticky="e", padx=(10, 0))
        ...
        def save():
            try:
                ...
                self.settings["ignored_coins"] = [c.strip().upper() for c in ignored_coins_var.get().split(",") if c.strip()]
    ...
    # ---- Telegram authentication login ----

    def _show_login_dialog(self) -> None:
        """Show Telegram authentication login dialog on startup"""
        login_window = tk.Toplevel(self)
        login_window.title("Telegram Authentication")
        ...
        if os.path.exists(logo_path):
            try:
                ...
                img = img.resize((150, 150), Image.Resampling.LANCZOS)
                photo = ImageTk.PhotoImage(img)
                logo_label = tk.Label(login_window, image=photo, bg=DARK_BG)
        ...
        # Instructions
        info_text = "Telegram Authentication\n\n1. Click 'Request Code'\n2. Check your Telegram\n3. Enter the 6-digit code"
        info_label = tk.Label(login_window, text=info_text, bg=DARK_BG, fg=DARK_FG, justify=tk.CENTER)
        info_label.pack(pady=10)
        
        # Code entry
        code_frame = ttk.Frame(login_window)
        code_frame.pack(pady=10)
        
        tk.Label(code_frame, text="Enter Code:", bg=DARK_BG, fg=DARK_FG).pack(side=tk.LEFT, padx=5)
        code_entry = tk.Entry(code_frame, width=15, font=("Arial", 16), justify=tk.CENTER)
        code_entry.pack(side=tk.LEFT, padx=5)
        code_entry.focus()
        
        # Status label
        status_label = tk.Label(login_window, text="", bg=DARK_BG, fg=DARK_ACCENT, font=("Arial", 9))
        status_label.pack(pady=5)
        
        # Store authentication code
        auth_code = {"value": None, "timestamp": 0}
        code_valid_minutes = 5
        
        def send_telegram_code():
            """Generate and send authentication code via Telegram"""
            try:
                # Load Telegram credentials
                token_path = os.path.join(self.project_dir, "telegram_token.txt")
                chat_id_path = os.path.join(self.project_dir, "telegram_chat_id.txt")
                
                if not os.path.exists(token_path) or not os.path.exists(chat_id_path):
                    status_label.config(text="Error: Telegram not configured", fg="red")
                    messagebox.showerror("Telegram Not Configured", 
                        "Please configure Telegram bot first:\n"
                        "- telegram_token.txt (your bot token)\n"
                        "- telegram_chat_id.txt (your chat ID)")
                    return
                ...
                if not bot_token or not chat_id:
                    status_label.config(text="Error: Telegram credentials empty", fg="red")
                    return
                
                # Generate 6-digit code
                code = str(random.randint(100000, 999999))
                auth_code["value"] = code
                auth_code["timestamp"] = time.time()
                
                # Send via Telegram
                url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
                message = f"🔐 PowerTrader AI Authentication Code\n\nYour login code is:\n\n{code}\n\nValid for {code_valid_minutes} minutes."
                payload = {
                    "chat_id": chat_id,
                    "text": message
                }
                ...
                def send_request():
                    """Send request in background thread, then update GUI on main thread"""
                    try:
                        resp = requests.post(url, json=payload, timeout=10)
                        resp_data = resp.json()
                        
                        # Schedule GUI updates on main thread - check if window still exists
                        ...
                        if resp.status_code == 200 and resp_data.get("ok"):
                            def update_success():
                                if login_window.winfo_exists():
                                    status_label.config(text="✓ Code sent to Telegram!", fg="green")
                            try:
                                login_window.after(0, update_success)
                            except:
                                pass  # Window might be closed
                            print(f"[AUTH] Code sent: {code}")
                        else:
                            error_msg = resp_data.get("description", resp.text) if resp_data else resp.text
                            error_code = resp_data.get("error_code", "Unknown")
                            
                            detailed_error = f"Telegram API Error ({error_code}):\n{error_msg}\n\n"
                            if error_code == 400:
                                detailed_error += "• Check that chat_id is correct (should be a number or start with -)\n"
                                detailed_error += f"• Your chat_id: {chat_id}\n"
                            elif error_code == 401:
                                detailed_error += "• Bot token is invalid or expired\n"
                                detailed_error += "• Get a new token from @BotFather\n"
                            elif error_code == 403:
                                detailed_error += "• Bot may be blocked by user\n"
                                detailed_error += "• Or chat_id is incorrect\n"
                            else:
                                detailed_error += "• Check bot token and chat ID\n"
                                detailed_error += "• Make sure bot can send messages to this chat\n"
                            
                            def update_error():
                                if login_window.winfo_exists():
                                    status_label.config(text="Error sending code", fg="red")
                                    messagebox.showerror("Failed to Send Code", detailed_error)
                            try:
                                login_window.after(0, update_error)
                            except:
                                pass
                            print(f"[AUTH ERROR] {error_code}: {error_msg}")
                            
                    except requests.exceptions.Timeout:
                        def update_timeout():
                            if login_window.winfo_exists():
                                status_label.config(text="Error: Timeout", fg="red")
                                messagebox.showerror("Connection Timeout", 
                                    "Request timed out. Check your internet connection and try again.")
                        try:
                            login_window.after(0, update_timeout)
                        except:
                            pass
                        print("[AUTH ERROR] Request timeout")
                    except requests.exceptions.ConnectionError as e:
                        def update_connection_error():
                            if login_window.winfo_exists():
                                status_label.config(text="Error: No connection", fg="red")
                                messagebox.showerror("Connection Error", 
                                    f"Cannot connect to Telegram API:\n{e}\n\nCheck your internet connection.")
                        try:
                            login_window.after(0, update_connection_error)
                        except:
                            pass
                        print(f"[AUTH ERROR] Connection error: {e}")
                    except Exception as e:
                        def update_unexpected_error():
                            if login_window.winfo_exists():
                                status_label.config(text="Error: Unexpected error", fg="red")
                                error_details = f"Unexpected error:\n{type(e).__name__}: {e}\n\nURL: {url}\nChat ID: {chat_id}"
                                messagebox.showerror("Error", error_details)
                        try:
                            login_window.after(0, update_unexpected_error)
                        except:
                            pass
                ...
                status_label.config(text="Sending code...", fg=DARK_ACCENT)
                
            except Exception as e:
                status_label.config(text="Error generating code", fg="red")
        ...
        def verify_code():
            ...
            if not auth_code["value"]:
                messagebox.showwarning("No Code", "Please request a code first")
                return
            
            # Check if code expired
            elapsed = time.time() - auth_code["timestamp"]
            if elapsed > (code_valid_minutes * 60):
                messagebox.showerror("Code Expired", f"Code expired. Please request a new one.")
                auth_code["value"] = None
                return
            
            # Normalize both codes - remove any non-digit characters and strip whitespace
            import re
            expected_code = re.sub(r'[^\d]', '', str(auth_code["value"]).strip())
            entered_code = re.sub(r'[^\d]', '', str(entered_code).strip())
            
            # Debug: print what we're comparing (only in console, not shown to user)
            print(f"[AUTH DEBUG] Entered (normalized): '{entered_code}' (len={len(entered_code)})")
            print(f"[AUTH DEBUG] Expected (normalized): '{expected_code}' (len={len(expected_code)})")
            print(f"[AUTH DEBUG] Match: {entered_code == expected_code}")
            
            # Verify code
            if entered_code == expected_code:
                status_label.config(text="✓ Authentication successful!", fg="green")
                print(f"[AUTH] Code verified successfully!")
                login_window.after(500, login_window.destroy)
            else:
                status_label.config(text="✗ Invalid code", fg="red")
                code_entry.delete(0, tk.END)
                code_entry.focus()
                print(f"[AUTH] Code mismatch - entered '{entered_code}' but expected '{expected_code}'")
                messagebox.showerror("Invalid Code", f"The code you entered is incorrect.\n\nExpected: {expected_code}\nEntered: {entered_code}\n\nPlease try again.")
        
        # Buttons
        btn_frame = ttk.Frame(login_window)
        btn_frame.pack(pady=15)
        
        request_btn = tk.Button(btn_frame, text="Request Code", command=send_telegram_code, width=15)
        request_btn.pack(side=tk.LEFT, padx=5)
        
        verify_btn = tk.Button(btn_frame, text="Verify Code", command=verify_code, width=15)
    ...
    def _show_about_dialog(self) -> None:
        ...
        if os.path.exists(logo_path):
            try:
                ...
                img = img.resize((150, 150), Image.Resampling.LANCZOS)
                photo = ImageTk.PhotoImage(img)
                logo_label = tk.Label(about_window, image=photo, bg=DARK_BG)
                logo_label.image = photo
                logo_label.pack(pady=10)
            except Exception as e:
                print(f"Error loading logo: {e}")
...
Supported by DogFun"""
        ...
        text_label = tk.Label(about_window, text=about_text, justify=tk.LEFT, padx=20, pady=10, bg=DARK_BG, fg=DARK_FG)
        text_label.pack()
        
        # Close button
        close_btn = tk.Button(about_window, text="Close", command=about_window.destroy, width=15)
        close_btn.pack(pady=10)

    def start_telegram_bot(self) -> None:
        """Start Telegram bot process"""
        if self.proc_telegram_bot.proc and self.proc_telegram_bot.proc.poll() is None:
            return
        if not os.path.isfile(self.proc_telegram_bot.path):
            messagebox.showerror("Missing script", f"Cannot find: {self.proc_telegram_bot.path}")
            return
        ...
        try:
            self.proc_telegram_bot.proc = subprocess.Popen(
                [sys.executable, "-u", self.proc_telegram_bot.path],
                cwd=self.project_dir,
                env=env,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
            ...
            messagebox.showerror("Failed to start", f"Telegram Bot failed to start:\n{e}")
    
    def stop_telegram_bot(self) -> None:
        """Stop Telegram bot process"""
        if not self.proc_telegram_bot.proc or self.proc_telegram_bot.proc.poll() is not None:
            return
        try:
            self.proc_telegram_bot.proc.terminate()
        except Exception:
            pass
    ...
    def _update_account_balance(self) -> None:
        """Update account balance from Binance API - optimized for performance"""
        try:
            # Import PowerTrader to get account info
            import sys
            import os
            trader_path = os.path.join(self.project_dir, "pt_trader.py")
            if os.path.isfile(trader_path):
                ...
                try:
                    ...
                    # Get account info (this is fast - single API call)
                    account = trader.get_account()
                    
                    if account:
                        # Update account labels immediately with EUR balance
                        eur_balance = account.get('balance', 0)
                        buying_power = account.get('buying_power', 0)
                        
                        self.lbl_acct_buying_power.config(text=f"Buying Power: {buying_power:.2f} EUR")
                        
                        # Get holdings (limit to avoid too many API calls)
                        holdings = trader.get_holdings()[:15]  # Limit to first 15 holdings
                        
                        # Calculate holdings value - skip dust amounts to speed things up
                        holdings_value = 0
                        for holding in holdings:
                            asset = holding['asset']
                            quantity = holding['free'] + holding['locked']
                            if quantity > 0.0001:  # Skip dust amounts
                                # Get price for this asset in EUR (skip if no EUR pair available)
                                try:
                                    price = trader.get_price(asset)
                                    if price and price > 0:
                                        holdings_value += quantity * price
                                except Exception:
                                    # Asset doesn't have EUR trading pair on Binance, skip it
                                    pass
                        
                        total_value = eur_balance + holdings_value
                        self.lbl_acct_total_value.config(text=f"Total Account Value: {total_value:.2f} EUR")
                        self.lbl_acct_holdings_value.config(text=f"Holdings Value: {holdings_value:.2f} EUR")
                        
                        # Calculate percent in trade
                        if total_value > 0:
                            percent_in_trade = (holdings_value / total_value) * 100
                            self.lbl_acct_percent_in_trade.config(text=f"Percent In Trade: {percent_in_trade:.2f}%")
                        else:
                            self.lbl_acct_percent_in_trade.config(text="Percent In Trade: 0.00%")
                    else:
                        # API error or no account data - don't overwrite existing values if they exist
                        current_text = self.lbl_acct_total_value.cget("text")
                        if "N/A" in current_text or "Error" in current_text:
                            self.lbl_acct_total_value.config(text="Total Account Value: Error fetching")
                            self.lbl_acct_holdings_value.config(text="Holdings Value: Error fetching")
                            self.lbl_acct_buying_power.config(text="Buying Power: Error fetching")
                except ImportError as e:
                    # pt_trader not available
                    pass
                except Exception as e:
                    # Error fetching account - show error
                    error_msg = str(e)[:50] if e else "Unknown error"
                    self.lbl_acct_total_value.config(text=f"Total Account Value: Error ({error_msg})")
        except Exception:
            pass
        
        # Schedule next update in 30 seconds (reduced frequency for better performance)
        try:
            self.after(30000, self._update_account_balance)
        except Exception:
            pass

    def test_buy_coin(self) -> None:
        """Test buy selected coin - manual trade for testing with user-specified amount"""
        try:
            import sys
            import os
            ...
            # Get selected coin
            selected_coin = self.buy_coin_var.get()
            if not selected_coin:
                messagebox.showwarning("Warning", "No coin selected to buy.")
                return
            
            trader_path = os.path.join(self.project_dir, "pt_trader.py")
            if not os.path.isfile(trader_path):
                messagebox.showerror("Error", "pt_trader.py not found")
                return
            ...
            # Get account to check balance and buying power
            account = trader.get_account()
            if not account:
            ...
            amount_str = simpledialog.askstring(
                f"Test Buy {selected_coin}",
                f"Enter purchase amount in EUR:\n\n"
                f"Available Balance: {balance:.2f} EUR\n"
                f"Buying Power: {buying_power:.2f} EUR\n\n"
                f"Amount (EUR):",
                initialvalue=str(min(buying_power, 10.0))  # Default to 10 EUR or buying power, whichever is less
            )
            ...
            if test_amount > balance:
                messagebox.showerror("Error", f"Insufficient balance. You have {balance:.2f} EUR, requested {test_amount:.2f} EUR")
                return
            
            # Get current coin price first (needed to check minimum notional)
            coin_price = trader.get_price(selected_coin)
            if not coin_price or coin_price <= 0:
                messagebox.showerror("Error", f"Failed to get {selected_coin} price")
                return
            
            # Check minimum notional requirement from Binance
            try:
                binance_symbol = f"{selected_coin}{trader.base_currency}"
                min_notional = trader._get_notional_filter(binance_symbol)
                if test_amount < min_notional:
                    messagebox.showerror(
                        "Amount Too Small",
                        f"Order amount {test_amount:.2f} EUR is below Binance's minimum notional requirement of {min_notional:.2f} EUR for {selected_coin}.\n\n"
                        f"Please enter at least {min_notional:.2f} EUR."
                    )
                    return
            except Exception as e:
                # If we can't get the filter, warn but continue (the trader will catch it)
                print(f"Warning: Could not check minimum notional: {e}")
            
            # For test buys, allow smaller amounts (minimum 1 EUR instead of min_trade_size)
            # But we've already checked Binance's minimum notional above
            if test_amount < 1.0:
                messagebox.showerror("Error", "Amount too small. Minimum is 1.00 EUR")
                return
            
            # Calculate quantity
            quantity = test_amount / coin_price
            
            # Confirm with user
            confirm = messagebox.askyesno(
                f"Confirm Test Buy {selected_coin}",
                f"Test Buy {selected_coin}:\n\n"
                f"Amount: {test_amount:.2f} EUR\n"
                f"Price: {coin_price:.4f} EUR\n"
                f"Quantity: {quantity:.8f} {selected_coin}\n\n"
                f"Proceed with test buy?"
            )
            
            if not confirm:
                return
            
            # Place buy order with better error handling
            try:
                order = trader.place_buy_order(selected_coin, quantity=quantity)
                
                if order:
                    executed_qty = float(order.get('executedQty', 0) or quantity)
                    # For MARKET orders, calculate average price from total cost / quantity
                    ...
                    if executed_qty > 0 and cummulative_quote_qty > 0:
                        executed_price = cummulative_quote_qty / executed_qty
                    else:
                    ...
                    try:
                        hub_dir = os.environ.get("POWERTRADER_HUB_DIR") or os.path.join(self.project_dir, "hub_data")
                        ...
                        print(f"Error writing to trade history: {e}")
                    
                    messagebox.showinfo(
                        "Success",
                        f"✅ Test Buy Executed!\n\n"
                        f"Coin: {selected_coin}\n"
                        f"Quantity: {executed_qty:.8f} {selected_coin}\n"
                        f"Average Price: {executed_price:.4f} EUR\n"
                        f"Total Cost: {total_cost:.2f} EUR"
                    )
                    # Refresh account balance and positions
                    self._update_account_balance()
                    self._refresh_positions_from_binance()
                else:
                    messagebox.showerror("Error", "Buy order failed. Order returned None.\n\nCheck console/terminal for error details.")
            except Exception as order_error:
                error_msg = str(order_error)
                # Show detailed error
                messagebox.showerror(
                    "Buy Order Error",
                    f"Failed to place buy order:\n\n{error_msg}\n\n"
                    f"Details:\n"
                    f"  Coin: {selected_coin}\n"
            ...
            messagebox.showerror("Error", f"Test buy failed:\n{str(e)}")
            import traceback
            traceback.print_exc()

    def sell_all_holdings(self) -> None:
        """Sell all holdings - manual sell for all coins"""
        try:
            import sys
            import os
            ...
            if not holdings:
                messagebox.showinfo("Info", "No holdings to sell")
                return
            
            # Filter out dust (very small amounts)
            significant_holdings = []
            for holding in holdings:
                quantity = holding['free'] + holding['locked']
                if quantity > 0.0001:  # Skip dust
                    # Check if it has a price (has EUR pair)
                    try:
                        price = trader.get_price(holding['asset'])
                        if price and price > 0:
                            value = quantity * price
                            if value >= 0.01:  # Only sell if worth at least 0.01 EUR
            ...
            if not significant_holdings:
                messagebox.showinfo("Info", "No significant holdings to sell")
                return
            
            # Show summary
            total_value = sum(h['value'] for h in significant_holdings)
            holdings_list = "\n".join([f"  • {h['asset']}: {h['quantity']:.4f} (≈{h['value']:.2f} EUR)" for h in significant_holdings])
            ...
            for holding in significant_holdings:
                try:
                    order = trader.place_sell_order(holding['asset'], holding['quantity'])
                    if order:
                        sold_count += 1
                        print(f"[GUI] Sold {holding['asset']}: {holding['quantity']:.4f}")
                    else:
                        failed_count += 1
                        print(f"[GUI] Failed to sell {holding['asset']}")
                except Exception as e:
                    failed_count += 1
                    print(f"[GUI] Error selling {holding['asset']}: {e}")
            
            # Show result
            if sold_count > 0:
                messagebox.showinfo(
                    "Sell All Complete",
                    f"✅ Sold {sold_count} coin(s)\n"
                    f"{'⚠️ Failed: ' + str(failed_count) if failed_count > 0 else ''}"
                )
            else:
                messagebox.showerror("Error", f"Failed to sell any holdings. Check logs for details.")
            
            # Refresh account balance
            self._update_account_balance()
                
        except Exception as e:
            messagebox.showerror("Error", f"Sell all failed:\n{str(e)}")
            import traceback
            traceback.print_exc()

    def sell_selected_coin(self) -> None:
        """Sell selected coin from dropdown - allows user to specify amount"""
        try:
            import sys
            import os
            from tkinter import messagebox, simpledialog
            
            coin = self.sell_coin_var.get().strip().upper()
            if not coin:
                messagebox.showerror("Error", "Please select a coin to sell")
                return
            ...
            coin_holding = None
            for holding in holdings:
                if holding['asset'].upper() == coin:
                    quantity = holding['free'] + holding['locked']
                    if quantity > 0.0001:  # Skip dust
                        coin_holding = holding
                        break
            
            if not coin_holding:
                messagebox.showinfo("Info", f"No {coin} holdings to sell")
                return
            
            available_quantity = coin_holding['free'] + coin_holding['locked']
            
            # Get current price
            price = trader.get_price(coin)
            if not price or price <= 0:
                messagebox.showerror("Error", f"Failed to get {coin} price")
                return
            
            total_value = available_quantity * price
            
            # Ask user how much to sell
            sell_amount_str = simpledialog.askstring(
                f"Sell {coin}",
                f"Available: {available_quantity:.8f} {coin} (≈{total_value:.2f} EUR)\n\n"
                f"Enter amount to sell:\n"
                f"  • Percentage (e.g., '50' for 50%)\n"
                f"  • Quantity (e.g., '0.5' for 0.5 {coin})\n"
                f"  • EUR amount (e.g., '100' for 100 EUR)\n\n"
                f"Amount:",
                initialvalue="100"  # Default to 100% (sell all)
            )
            
            if not sell_amount_str:
                return  # User cancelled
            
            sell_amount_str = sell_amount_str.strip()
            
            # Parse the input - determine if it's percentage, quantity, or EUR amount
            try:
                sell_value = float(sell_amount_str)
            except ValueError:
                messagebox.showerror("Error", "Invalid amount. Please enter a number.")
                return
            
            # Determine what the user entered based on the value
            # If it's > 1 and <= 100, assume percentage
            # If it's > 100, assume EUR amount
            # Otherwise, assume quantity
            if 0 < sell_value <= 100:
                # Percentage
                sell_percentage = sell_value
                sell_quantity = (available_quantity * sell_percentage) / 100.0
                sell_type = "percentage"
            elif sell_value > 100:
                # EUR amount
                sell_quantity = sell_value / price
                sell_type = "EUR amount"
            else:
                # Quantity
            ...
            if sell_quantity > available_quantity:
                messagebox.showerror(
                    "Error",
                    f"Requested quantity {sell_quantity:.8f} {coin} exceeds available {available_quantity:.8f} {coin}"
                )
                return
            
            if sell_quantity <= 0:
                messagebox.showerror("Error", "Sell quantity must be greater than 0")
                return
            
            # Calculate sell value in EUR
            sell_value_eur = sell_quantity * price
            
            # Confirm with user
            confirm_msg = f"Sell {coin}:\n\n"
            if sell_type == "percentage":
                confirm_msg += f"Percentage: {sell_percentage:.1f}%\n"
            elif sell_type == "EUR amount":
                confirm_msg += f"EUR Amount: {sell_value:.2f} EUR\n"
            confirm_msg += f"Quantity: {sell_quantity:.8f} {coin}\n"
            confirm_msg += f"Price: {price:.4f} EUR\n"
            confirm_msg += f"Total: {sell_value_eur:.2f} EUR\n\n"
            confirm_msg += f"Proceed with sell?"
            
            confirm = messagebox.askyesno("Confirm Sell", confirm_msg)
            ...
            # Place sell order
            try:
                order = trader.place_sell_order(coin, sell_quantity)
                
                if order:
                    executed_qty = float(order.get('executedQty', sell_quantity))
                    # For MARKET orders, calculate average price from total cost / quantity
                    cummulative_quote_qty = float(order.get('cummulativeQuoteQty', 0) or order.get('cummulativeFilledQuoteQty', 0) or 0)
                    if executed_qty > 0 and cummulative_quote_qty > 0:
                        executed_price = cummulative_quote_qty / executed_qty
                    ...
                    total_received = executed_qty * executed_price
                    
                    messagebox.showinfo(
                        "Success",
                        f"✅ Sell Executed!\n\n"
                        f"Coin: {coin}\n"
                        f"Quantity: {executed_qty:.8f} {coin}\n"
                        f"Average Price: {executed_price:.4f} EUR\n"
                        f"Total Received: {total_received:.2f} EUR"
                    )
                    # Refresh account balance and positions
                    self._update_account_balance()
                    self._refresh_positions_from_binance()
                else:
                    messagebox.showerror("Error", f"Sell order failed. Check logs for details.")
            except Exception as order_error:
                error_msg = str(order_error)
                messagebox.showerror(
                    "Sell Order Error",
                    f"Failed to place sell order:\n\n{error_msg}\n\n"
                    f"Details:\n"
                    f"  Coin: {coin}\n"
                    f"  Quantity: {sell_quantity:.8f}\n"
                    f"  Price: {price:.4f} EUR\n"
                    f"  Value: {sell_value_eur:.2f} EUR"
                )
                import traceback
                traceback.print_exc()
                
        except Exception as e:
            messagebox.showerror("Error", f"Sell failed:\n{str(e)}")
            import traceback
            traceback.print_exc()
    
    def set_trade_amount(self) -> None:
        """Set the fixed trade amount (EUR per trade)"""
        try:
            from tkinter import messagebox, simpledialog
            
            # Load current settings
            settings_path = os.path.join(self.project_dir, "gui_settings.json")
            current_amount = 6.0  # Default
            
            if os.path.exists(settings_path):
                try:
                    with open(settings_path, 'r', encoding='utf-8') as f:
                        settings = json.load(f)
                        current_amount = settings.get("fixed_trade_amount", 6.0)
                except Exception:
                    pass
            
            # Ask user for new trade amount
            amount_str = simpledialog.askstring(
                "Set Trade Amount",
                f"Enter the fixed trade amount in EUR:\n\n"
                f"This is the amount the bot will use for each trade.\n"
                f"Current: {current_amount:.2f} EUR\n\n"
                f"New amount (EUR):",
                initialvalue=str(current_amount)
            )
            ...
            if new_amount < 1.0:
                messagebox.showerror("Error", "Trade amount must be at least 1.00 EUR")
                return
            ...
            settings["fixed_trade_amount"] = new_amount
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success",
                    f"✅ Trade amount set to {new_amount:.2f} EUR per trade\n\n"
                    f"The trader will use this amount for all future trades."
                )
                # Update the display immediately
                self.lbl_acct_trade_amount.config(text=f"Trade Amount: {new_amount:.2f} EUR")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save settings:\n{str(e)}")
    ...
    def _refresh_positions_from_binance(self) -> None:
        """Manually refresh positions from Binance and update trader_status.json"""
        try:
            import sys
            if self.project_dir not in sys.path:
                sys.path.insert(0, self.project_dir)
            ...
            # Get current holdings from Binance
            holdings = trader.get_holdings()
            account = trader.get_account()
            
            if not holdings or not account:
                return
            ...
            # Calculate positions from holdings
            positions = {}
            for holding in holdings:
                asset = holding['asset']
                if asset == 'EUR':
            ...
            # Update trader_status.json with positions
            try:
                ...
                if account:
                    ...
                    status['account'] = {
                        ...
                        "holdings_sell_value": round(holdings_sell_value, 2),
                        "percent_in_trade": round(percent_in_trade, 2)
                    }
                ...
                with open(self.trader_status_path, 'w', encoding='utf-8') as f:
                    json.dump(status, f, indent=2)
                
                # Force GUI refresh by clearing mtime cache
                self._last_trader_status_mtime = None
                
            except Exception as e:
                print(f"Error updating trader_status.json: {e}")
                import traceback
                traceback.print_exc()
                
        except Exception as e:
            print(f"Error refreshing positions from Binance: {e}")
            import traceback
            traceback.print_exc()
