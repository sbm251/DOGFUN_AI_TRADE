...
# Try to import kucoin, with helpful error message if missing
# Try Client first (newer API), fall back to Market (older API)
try:
    from kucoin.client import Client
    market = Client()
    _kucoin_api_type = "Client"
except ImportError:
    try:
        from kucoin.client import Market
        market = Market(url='https://api.kucoin.com')
        _kucoin_api_type = "Market"
    except ImportError as e:
        print(f"[ERROR] Failed to import kucoin module: {e}")
        print(f"[ERROR] Python executable: {sys.executable}")
        print(f"[ERROR] Python version: {sys.version}")
        print(f"[ERROR] Python path: {sys.path[:3]}...")  # Show first 3 paths
        print(f"[ERROR] Current working directory: {os.getcwd()}")
        print(f"[ERROR] Script location: {os.path.dirname(os.path.abspath(__file__))}")
        print(f"[ERROR] Please install kucoin-python: {sys.executable} -m pip install kucoin-python")
        sys.exit(1)

# Try to import telegram_notifications, but make it optional
try:
    # First try importing from current directory
    from telegram_notifications import notify_training_complete
except ImportError:
    try:
        # If that fails, try adding project directory to path
        ...
        # If still can't import, create a dummy function
        def notify_training_complete(message):
...
# Print startup message to confirm real trainer is running
print(f"[TRAINER] Starting real neural network training for {_arg_coin}")
print(f"[TRAINER] Using KuCoin API with pair: {coin_choice}")
...
while True:
	...
	while True:
		...
		try:
			...
			if _kucoin_api_type == "Client":
				# New API: get_kline_data
				kline_type = timeframe
				history_data = market.get_kline_data(coin_choice, kline_type=kline_type, start=end_time*1000, end=start_time*1000)
				# Convert to string format similar to old API
				history = str(history_data).replace(']]','], ').replace('[[','[').split('], [')
			else:
				# Old API: get_kline
		...
		if list_len == 0:
			print(f'[TRAINER] Gathering historical price data for {_arg_coin} ({coin_choice}) from KuCoin...')
		else:
	...
	try:
		...
		if _kucoin_api_type == "Client":
			...
			ticker_data = market.get_ticker(coin_choice)
		...
		while True:
			...
			if should_stop_training(loop_i):
				...
				try:
					...
					# Notify via Telegram
					try:
						notify_training_complete(f"{_arg_coin} training completed successfully")
					except:
						pass
				except:
					pass
